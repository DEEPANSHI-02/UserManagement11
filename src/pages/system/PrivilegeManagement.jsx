...
import { Plus, Edit, Trash2, Filter, Search, Key, List, Clock } from 'lucide-react';
...
const PrivilegeManagement = () => {
  ...
  const loadPrivileges = async () => {
    setLoading(true);
    try {
      const res = await systemAdminMockApi.getPrivileges();
  ...
  const loadAuditLog = async () => {
    try {
      const res = await systemAdminMockApi.getPrivilegeAuditLog();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editPrivilege) {
        await systemAdminMockApi.updatePrivilege(editPrivilege.id, form);
      } else {
        await systemAdminMockApi.createPrivilege(form);
      }
      closeModal();
      loadPrivileges();
      loadAuditLog();
  ...
  const handleDelete = async (priv) => {
    ...
    try {
      await systemAdminMockApi.deletePrivilege(priv.id);
      loadPrivileges();
      loadAuditLog();
    } catch {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded shadow p-6 mt-8">
        <h2 className="text-lg font-bold flex items-center gap-2 mb-4"><List className="h-5 w-5" /> Privilege Audit Log</h2>
          ...
          <table className="min-w-full text-xs">
            <thead>
              <tr className="bg-gray-50">
                ...
                <th className="p-2 text-left">Privilege</th>
                <th className="p-2 text-left">By</th>
              </tr>
            </thead>
            <tbody>
              {auditLog.length === 0 ? (
                <tr><td colSpan={4} className="p-3 text-center text-gray-400">No audit log entries.</td></tr>
              ) : auditLog.map((log, i) => (
                <tr key={i} className="border-b">
                  <td className="p-2 flex items-center gap-1"><Clock className="h-3 w-3 text-gray-400" /> {new Date(log.time).toLocaleString()}</td>
                  <td className="p-2">{log.action}</td>
                  <td className="p-2">{log.privilegeName}</td>
