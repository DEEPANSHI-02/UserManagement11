...
import { Plus, Edit, Trash2, Filter, Search, Building, FileText, CheckCircle, AlertTriangle, BarChart3, Clock } from 'lucide-react';
...
const LegalEntityManagement = () => {
  ...
  const loadEntities = async () => {
    setLoading(true);
    try {
      const res = await systemAdminMockApi.getLegalEntities();
  ...
  const loadAuditLog = async () => {
    try {
      const res = await systemAdminMockApi.getLegalEntityAuditLog();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editEntity) {
        await systemAdminMockApi.updateLegalEntity(editEntity.id, form);
      } else {
        await systemAdminMockApi.createLegalEntity(form);
      }
      closeModal();
      loadEntities();
      loadAuditLog();
  ...
  const handleDelete = async (entity) => {
    ...
    try {
      await systemAdminMockApi.deleteLegalEntity(entity.id);
      loadEntities();
      loadAuditLog();
    } catch {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded shadow p-6 mt-8">
        <h2 className="text-lg font-bold flex items-center gap-2 mb-4"><FileText className="h-5 w-5" /> Legal Entity Audit Log</h2>
